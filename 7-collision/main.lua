...
function love.update(dt)
    ...
    if scrolling then
        ...
        -- Cek kondisi pipe disetiap layar
        for k, pair in pairs(pipePairs) do
            pair:update(dt)

            -- Cek kondisi saat bird bertabrakan dengan pipe
            for l, pipe in pairs(pair.pipes) do
                if Bird:collides(pipe) then
                    -- Pause game saat terjadi tabrakan
                    scrolling = false
                end
            end

            -- jika pipe tidak terlihat dilayar makan pipe dihapus
